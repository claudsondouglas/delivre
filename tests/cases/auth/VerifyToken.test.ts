...
describe("Verificar token", () => {
    ...
    beforeAll(async () => {
        ...
        validToken = await new Authenticate(repository, hasher).execute('admin@email.com', '123456');
    });

    it("should be able to verify a valid token", async () => {
        expect(new Verify().execute(validToken)).resolves.toBeTruthy();
    });

    it("should not be able to verify an invalid token", async () => {
        expect(new Verify().execute(invalidToken)).rejects.toThrow();
    });
})